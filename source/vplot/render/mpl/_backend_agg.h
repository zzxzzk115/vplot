                                        ...
                                        ColorArray &edgecolors)
{
    ...
    DashesVector linestyles;
    /* An empty (0, 4) color array: quad meshes carry no per-path hatch colors,
       so the generic collection path falls back to gc.hatch_color. The original
       spelled this as an empty numpy array; a value-initialized view is the
       same thing here, since safe_first_shape() reports 0 either way. */
    ColorArray hatchcolors{};
